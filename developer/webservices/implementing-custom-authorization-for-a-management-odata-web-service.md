---
title: Egyéni engedélyezési végrehajtása egy felügyeleti OData webszolgáltatást |} A Microsoft Docs
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
ms.locfileid: "62080655"
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="39d2b-102">Egyéni hitelesítés implementálása egy Management OData webszolgáltatásban</span><span class="sxs-lookup"><span data-stu-id="39d2b-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="39d2b-103">A Windows PowerShell webszolgáltatás segítségével egy harmadik féltől származó megvalósításához szükséges a [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) felület Windows PowerShell-parancsmagok elérhetővé.</span><span class="sxs-lookup"><span data-stu-id="39d2b-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="39d2b-104">Ez az interfész a felhasználó engedélyezése a web Service hajt végre.</span><span class="sxs-lookup"><span data-stu-id="39d2b-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="39d2b-105">A felület megvalósítása a kód írása, után fordítani kell azokat a DLL-ek a webes alkalmazásban használható.</span><span class="sxs-lookup"><span data-stu-id="39d2b-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="39d2b-106">Az átmenő hitelesítési</span><span class="sxs-lookup"><span data-stu-id="39d2b-106">Pass-through authorization</span></span>

<span data-ttu-id="39d2b-107">A legegyszerűbben úgy valósítható meg a [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) felület, amely a minden felhasználó és a csatlakoztatott megvalósítás esetén.</span><span class="sxs-lookup"><span data-stu-id="39d2b-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="39d2b-108">Ebben a példában nincs biztonsági és csak olyan bemutatásáért, ahogy a felület megvalósítása megadott s biztosít.</span><span class="sxs-lookup"><span data-stu-id="39d2b-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="39d2b-109">Megvalósítását a [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) felület felül kell írnia a két módszer: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span><span class="sxs-lookup"><span data-stu-id="39d2b-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="39d2b-110">Ebben a példában a [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) mindig adja meg a **adatait a System.Security.Principal.WindowsIdentity** az aktuális felhasználóhoz társított objektumot.</span><span class="sxs-lookup"><span data-stu-id="39d2b-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="39d2b-111">Szerepkör-alapú hitelesítést</span><span class="sxs-lookup"><span data-stu-id="39d2b-111">Role-based authorization</span></span>

<span data-ttu-id="39d2b-112">Az alábbi példa egy szerepkör-alapú engedélyezési szabályzatot valósít meg.</span><span class="sxs-lookup"><span data-stu-id="39d2b-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="39d2b-113">A házirend XML-fájl a fő alkalmazását a Web.config fájl a könyvtárban található, és a MOF- és XML-leképezést sémafájlok van meghatározva.</span><span class="sxs-lookup"><span data-stu-id="39d2b-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="39d2b-114">Az engedélyezési sémafájl konfigurálásával kapcsolatos további információkért lásd: [konfigurálása szerepkör-alapú hitelesítést](./configuring-role-based-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="39d2b-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="39d2b-115">Az első rész mintát valósítja meg a [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) és [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) módszereket.</span><span class="sxs-lookup"><span data-stu-id="39d2b-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="39d2b-116">Ebben az esetben a illesztőmetódusait hívja metódusok a `RbacSystem` osztály (a meghatározást lásd alább), amelyeket a felhasználó engedélyeinek ellenőrzése a tényleges munkát.</span><span class="sxs-lookup"><span data-stu-id="39d2b-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="39d2b-117">A következő példában létrehozunk egy osztályt, amely elemzi az XML-fájlt az engedélyezési házirend.</span><span class="sxs-lookup"><span data-stu-id="39d2b-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="39d2b-118">A következő osztályok rendre jelölik csoportokat és felhasználókat.</span><span class="sxs-lookup"><span data-stu-id="39d2b-118">The following classes represent groups and users, respectively.</span></span>
...
<span data-ttu-id="39d2b-119">Végül a RbacSystem osztálya határozza meg a módszereket, amelyek a munkát, a felhasználó az engedélyek ellenőrzése és az engedélyezési állapotot térjen vissza a meghatározott végrehajtásának módszerek a [Microsoft.Management.Odata.CustomAuthorization ](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) felületet.</span><span class="sxs-lookup"><span data-stu-id="39d2b-119">Finally, the RbacSystem class implements methods that do the work of checking the permissions for the user and return the authorization status to the methods defined in the implementation of the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface.</span></span>